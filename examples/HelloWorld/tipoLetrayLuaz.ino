...
void loop() {
  ...
  Disp.loop(); //--> Run "Disp.loop" to refresh the LED
    ...
    if(p){
    Serial.println("P enviado a loop 1");
    m=1;
    p=false;
    }
  //Disp.drawString(0, 0, "            "); //--> Display text "Disp.drawText(x position, y position, text)";
    
  Scrolling_Text(m, 50,myString,tipoLetra); //--> Show running text "Scrolling_Text(y position, speed);"  
...
void Scrolling_Text(int n, uint8_t scrolling_speed,String data, String tipoLetra) {
  static uint32_t pM;
  static uint32_t a;
  static uint32_t x;
  static uint32_t y;
